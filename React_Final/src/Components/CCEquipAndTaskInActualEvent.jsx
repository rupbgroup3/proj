    ...
    setTimeout(
      function(){
        ...
        this.setState({ TaskClickedArr: newArr2,
          ...
          if(this.props.PrevLocation=='/ActualEvent'&& this.props.location.state!=undefined)  {
              
              for (let index = 0; index < this.props.Event_Type_EquipmentTypeVar.length; index++) {
                
                  for (let index2 = 0; index2 < this.props.EquipmentVar.length; index2++) {
                      const element2 = this.props.EquipmentVar[index2];
                      const element = this.props.Event_Type_EquipmentTypeVar[index];
                    
                      if (element.Event_TypeEventCode== EventCode&& element.EquipmentTypeCode== element2.Code ) {
              ...
              for (let index = 0; index < this.props.Event_Type_TaskVar.length; index++) {
                ...
                for (let index2 = 0; index2 < this.props.TasksTypeVar.length; index2++) {
                    const element2 = this.props.TasksTypeVar[index2];
                    const element = this.props.Event_Type_TaskVar[index];
                 
                    if (element.Event_TypeEventCode== EventCode&& element.TaskTaskNo== element2.TaskNo ) {
          ...
          else if(this.props.PrevLocation=='/EquipmentType'&& this.props.location.state!=undefined) {
            this.setState({
          ...
          else if(this.props.PrevLocation=='/TasksType'&& this.props.location.state!=undefined){
            this.setState({
                TaskClickedArr: this.props.location.state.TaskClickedArr,
                EquipClickedArr: this.props.location.state.EquipClickedArr,
                TasksTypeVar: this.props.location.state.TasksTypeVar,
  ...
  MoveToEquipTable=()=>{
    ...
    this.props.history.push({
      ...
      state:{TaskClickedArr: this.state.TaskClickedArr,
        ...
        Barcode: this.props.Actual_Event[this.props.Actual_Event.length-1].Barcode}
    })
  }
  MoveToTasksTable=()=>{
    this.props.GetActualEvent();
  ...
  DefiningTasksStepClicked=()=>{
    ...
    if(this.state.TaskClickedArr.length==0){
      ...
      Swal.fire({
        ...
        text: "בלחיצה על כן- סוגי המשימות וסוגי הציוד ששוייכו מחדש יישמרו לשימוש עתידי ויבחרו אוטומטית בהגדרת אותו סוג אירוע בעתיד",        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'כן',
