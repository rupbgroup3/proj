...
class CCEventType extends Component {
  ...
  componentDidMount() {
    ...
    setTimeout(
      function () {
        ...
        this.setState({
          rows: this.props.EventTypeVar.map((data) => ({
  ...
  EditTableShow = () => {
    this.setState({ EditTableShow: true, BackToEditButtons: false });
    setTimeout(
      function () {
        this.setState({
          rows: this.props.EventTypeVar.map((data) => ({
  ...
  editUser = (data) => {
    ...
    setTimeout(
      function () {
        for (
          ...
          if (element == EveCode) {
            EquipCodesArr.push(
              this.props.Event_Type_EquipmentTypeVar[index].EquipmentTypeCode
            );
          }
        }
        for (
          let index1 = 0;
          index1 < this.props.EquipmentVar.length;
          index1++
        ) {
    ...
    setTimeout(
      function () {
        for (
          let index1 = 0;
          index1 < this.props.TasksTypeVar.length;
          index1++
        ) {
  ...
  deleteChoose = async (data) => {
    ...
    const index2 = this.state.rows.findIndex(
      (user) => user.EventCode == data.EventCode
    );
    console.log(index2);
    const newArr2 = this.state.rows.filter((person, key) => key !== index2);
  ...
  deleteUser = async (data) => {
    Swal.fire({
      ...
      if (result.value) {
        ...
        Swal.fire("!נמחק", "פרטי האירוע נמחקו", "success");
      }
    });
  };
  ...
  PutExistEvent = async () => {
    if (
      ...
      this.state.EventNameEntered == null
    ) {
      Swal.fire({
        icon: "error",
      ...
      for (
        ...
        await this.props.DeleteEvent_Type_EquipmentType(
          Event_Type_EquipmentType
        );
      }
      for (
        let index = 0;
    ...
    if (
      ...
      setTimeout(
        function () {
          for (
            let index = 0;
            index < this.state.TaskClickedArr.length;
            index++
          ) {
            const element = this.state.TaskClickedArr[index];
            let Event_Type_Task = {
              ...
              <table>
                <tr>
                  <td id="tdLeft">
                    <div id="divEquipment">
                      <div id="one">
                        <h2 className="ClassH2">סוגי ציוד</h2>
                        <div id="two">
                          <MDBBtn
                            rounded
                            outline
                  ...
                  <td id="tdRight">
                    <div id="upperRight">
                      <div id="one">
                        <h2 className="ClassH2">סוגי משימות</h2>
                        <div id="two">
                          <MDBBtn
                            rounded
                            outline
