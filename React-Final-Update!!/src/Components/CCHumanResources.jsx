...
class CCHumanResources extends React.Component {
  ...
  deleteUser = (Email) => {
    Swal.fire({
      ...
      if (result.value) {
        Swal.fire("!נמחק", "פרטי האדם נמחקו", "success");
    ...
    setTimeout(
      function () {
        if (data.IsEmployee == 1) {
          ...
          for (
            let index = 0;
            index < this.props.Role_EmployeeVar.length;
            index++
          ) {
    ...
    if (!this.state.editMode) {
      if (this.state.ISEMPLOYEE === "true") {
        for (
          ...
          const element = this.props.Role_EmployeeVar[index];
          if (
            element.RoleCode == this.state.RoleEmployee &&
            element.RoleCode != "9"
          ) {
            e.preventDefault();
            Swal.fire({
              ...
              text:
                "אנא שנה את התפקיד או שנה את העובד הנמצא בתפקיד זה לתפקיד שונה או השתמש באופציית אחר",
            });
            return;
          }
        }
        ...
        function () {
          this.setState({
            rows: this.props.PersonVar.map((data) => ({
              ...
              DepartmnetDepartmentCode:
                data.DepartmnetDepartmentCode == 1
                  ? "הנדסה"
                  : data.DepartmnetDepartmentCode == 2
                  ? "כלכלה ומנהל עסקים"
                  : data.DepartmnetDepartmentCode == 3
                  ? "מדעי החברה והקהילה"
                  : data.DepartmnetDepartmentCode == 4
                  ? "מדעי הים"
                  : data.DepartmnetDepartmentCode == 5
                  ? "הנדסאים- המכללה הטכנולוגית"
                  : data.DepartmnetDepartmentCode == 6
                  ? "מכינות"
                  : "אחר",
              ActiveTillYear: data.ActiveTillYear,
              StudyingStartyear: data.StudyingStartyear,
              ...
              actions: (
                <div style={{ textAlign: "center" }}>
                  ...
                  <MDBBtn
                    onClick={() => this.deleteUser(data.Email)}
                    color="danger"
                  >
                    {" "}
                    <img src="https://img.icons8.com/material/25/000000/delete--v1.png" />
                  </MDBBtn>
                  <MDBBtn onClick={() => this.TotalHours(data)} color="info">
                    {" "}
                    <img src="https://img.icons8.com/material/25/000000/clock--v1.png" />
                  </MDBBtn>{" "}
                </div>
              ),
            })),
      ...
      if (this.state.ISEMPLOYEE === "true") {
        for (
          ...
          if (
            element.RoleCode == this.state.RoleEmployee &&
            element.EmployeeEmpCode != this.state.Empcode &&
            element.RoleCode != "9"
          ) {
            e.preventDefault();
            Swal.fire({
            ...
            actions: (
              <div id="colAction">
                <MDBBtn
                  ...
                  color="warning"
                >
                  <img src="https://img.icons8.com/android/20/000000/edit.png" />
                </MDBBtn>
                <MDBBtn
                  id="BTN3_table"
                  onClick={() => this.deleteUser(data.Email)}
                ...
                >
                  {" "}
                  <img src="https://img.icons8.com/material/20/000000/delete--v1.png" />
                </MDBBtn>
                <MDBBtn
                  id="BTN3_table"
                  onClick={() => this.TotalHours(data)}
                  color="info"
                >
                  {" "}
                  <img src="https://img.icons8.com/material/20/000000/clock--v1.png" />
                </MDBBtn>
              </div>
            ),
          })),
