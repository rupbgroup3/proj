...
public class DBservices
{
    ...
    public List<TotalTasksPer> GetTotalTasksPer()
    {
        ...
        try
        {
            ...
            String selectSTR = "select * from Task_Person tp inner join Task t on tp.TaskTaskNo= t.TaskNo inner join person p on tp.PersonEmpCode= p.EmpCode order by tp.exceutionDate";
            SqlCommand cmd = new SqlCommand(selectSTR, con);
    ...
    public List<TotalTasksInEventPer> GetTotalTasksInEventPer()
    {
        ...
        try
        {
            ...
            String selectSTR = "select * from Person_plan_TasksInEvent ppt inner join Task t on ppt.plan_TasksInEventTaskTaskNo= t.TaskNo inner join person p on ppt.PersonEmpCode=p.EmpCode order by ppt.executionDate";
            SqlCommand cmd = new SqlCommand(selectSTR, con);
  ...
  where tp.exceutionDate>= '" + FromDate + "' and tp.exceutionDate <= '" + ToDate + "' group by tp.PersonEmpCode, p.EmpFirstName, p.EmpLastName, d.DepartmentName";
    ...
    {
        List<CalendarActualEvent> CalendarActualEvent = new List<CalendarActualEvent>();
        SqlConnection con = null;
        ...
        try
        {
            ...
            while (dr.Read())
            {   // Read till the end of the data into a row
                CalendarActualEvent f = new CalendarActualEvent();

                f.Barcode = (int)dr["Barcode"];
                f.date = (string)dr["date"];
                f.FromHour = (double)dr["FromHour"];
                f.ToHour = (double)dr["ToHour"];
                f.EventName = (string) dr["EventName"];
                f.EmpFirstName = (string) dr["EmpFirstName"];
                f.EmpLastName = (string) dr["EmpLastName"];
                f.Location = (string) dr["Location"];
                f.EmployeeEmpCode = (int)dr["EmployeeEmpCode"];
                f.Event_TypeEventCode = (int)dr["Event_TypeEventCode"];

                CalendarActualEvent.Add(f);
            }

            return CalendarActualEvent;
        }
        catch (Exception ex)
        {
            // write to log
    ...
    {
        List<CalendarActualTask> CalendarActualTask = new List<CalendarActualTask>();
        SqlConnection con = null;
        ...
        try
        {
            ...
            String selectSTR = @"  select * from Task_Person tp inner join Task t on tp.TaskTaskNo= t.TaskNo inner join person p on tp.PersonEmpCode= p.EmpCode where tp.exceutionDate= '" + FromDate + "'";
            SqlCommand cmd = new SqlCommand(selectSTR, con);
            ...
            while (dr.Read())
            {   // Read till the end of the data into a row
                CalendarActualTask f = new CalendarActualTask();

                f.TaskTaskNo = (int)dr["TaskTaskNo"];
                f.exceutionDate = (string)dr["exceutionDate"];
                f.FromHour = (double)dr["FromHour"];
                f.ToHour = (double)dr["ToHour"];
                f.TaskName = (string)dr["TaskName"];
                f.EmpFirstName = (string)dr["EmpFirstName"];
                f.EmpLastName = (string)dr["EmpLastName"];
                f.PersonEmpCode = (int)dr["PersonEmpCode"];

                CalendarActualTask.Add(f);
            }

            return CalendarActualTask;
        }
        catch (Exception ex)
        {
            // write to log
    ...
    public List<CalendarActualTask> GetCalendarActualTaskRangeDate(string FromDate, string ToDate)
    {
        ...
        try
        {
            ...
            String selectSTR = @"select * from Task_Person tp inner join Task t on tp.TaskTaskNo= t.TaskNo inner join person p on tp.PersonEmpCode= p.EmpCode where tp.exceutionDate >= '" + FromDate + "' and tp.exceutionDate<= '" + ToDate + "'";
            SqlCommand cmd = new SqlCommand(selectSTR, con);
